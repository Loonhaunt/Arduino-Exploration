/***
  Read Our Complete Guide: https://RandomNerdTutorials.com/bme680-sensor-arduino-gas-temperature-humidity-pressure/
   1 21 2024 Dave Brown Modified sketch Serial.println to reflect calculated output in Temp degree F, Pressure Hg, Altitude in Feet Above Sealevel, FAS.
     Change Delay to five seconds.  
    Running knock off BME requires 5v or not able to read returns error in most cases
...
void loop() {
  ...
  Serial.print((bme.pressure / 100.0)*.030);
  Serial.println(F(" Hg"));
  ...
  Serial.println(F(" FAS"));

  Serial.println();
  delay(5000); //Delay 5 min 
}
